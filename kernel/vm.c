...
// Create a per-process kernel page for a specific process, called in allocproc
...
// Because pp kpagetable contains mappings of the kernel code, kernel data, we are
// forbidden to free the actual code/data in the kernel part. The only actual data 
// we will free pp kstack, will resides only in ppkpagetable. The other user address
// space data will be freeed when freeing user page table, so we only free the mappings here.

// In the end, we free mappings and data for kstack, free mappings of other kerlnel part and user part.
// To the kpagetable's end, all the leaves can be treated as freeed, we can call freewalk to it.
void pp_kvmfree(pagetable_t kpt, uint64 kstack, uint64 sz)
{
  ...
  // the trampoline for trap entry/exit to
  // the highest virtual address in the kernel.
  uvmunmap(kpt, TRAMPOLINE, 1, 0);

  // Free per process kernel stack mapping and data
  uvmunmap(kpt, kstack, 1, 1);

  // Free mappings for user address space
...
// Copy the per-process page table mappings to ppkpagetable, so that 
// the low address generated by ppkpagetable can reference to data in
// process address space. Note: only copy mappings from pt to kpt, not the data.
int uvmcopy2kpagetable(pagetable_t pt, pagetable_t kpt, uint64 sz)
{
  ...
  for(i = 0; i < sz; i += PGSIZE){
    ...
    if((pte = walk(pt, i, 0)) == 0)
      panic("uvmcopy2kpagetable: pte should exist");
    if((*pte & PTE_V) == 0)
      panic("uvmcopy2kpagetable: page not present");
    pa = PTE2PA(*pte);
    flags = PTE_FLAGS(*pte);

    // Map the pte in kpagetable


    
    if(mappages(kpt, i, PGSIZE, pa, flags) != 0)
      panic("uvmcopy2kpagetable: mapping from pt to kpt errorss");
  }
  return 0;
}
