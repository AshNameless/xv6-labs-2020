...
// Copy the per-process page table mappings to ppkpagetable from start va to end va.
// Note: only copy mappings from pt to kpt, not the data.
int uvm2kpagetable(pagetable_t pt, pagetable_t kpt, uint64 start, uint64 end)
{
  ...
  // printf("map %d's pagetable to kpagetable\n", myproc()->pid);
  // printf("page table:\n");
  // vmprint(myproc()->pagetable);
  // printf("kpagetable:\n");
  // vmprint(myproc()->kpagetable);

  for(i = start; i < end; i += PGSIZE){
    // Find the pte entry
    if((pte = walk(pt, i, 0)) == 0)
      panic("uvm2kpagetable: pte should exist");
    if((*pte & PTE_V) == 0)
      panic("uvm2kpagetable: page not present");
